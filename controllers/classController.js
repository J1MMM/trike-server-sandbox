...
const createClass = async (req, res) => {
    ...
    if (!section || !gradeLevel || !schoolYear) return res.status(400).json({ "message": "All fields are required" })
    const teacherID = req.id;
    ...
    try {
        ...
        res.status(201).json({ "success": `New Class '${section}' has been created successfully!`, result })
...
const archiveClass = async (req, res) => {
    ...
    if (!id || !req.id) return res.status(400).json({ 'message': "ID's are required" })
